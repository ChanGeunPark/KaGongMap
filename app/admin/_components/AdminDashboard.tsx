...
export default function AdminDashboard() {
  ...
  const TAB_QUERY: Record<Exclude<FilterTab, "reports">, { isLoading: boolean; isError: boolean; isEmpty: boolean }> = {
    pending: {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7]">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        {currentTab && !currentTab.isLoading && !currentTab.isError && currentTab.isEmpty && (
          <div className="flex flex-col items-center py-24 text-gray-400">
            <span className="text-4xl mb-3">📭</span>
            <p className="text-sm">{EMPTY_LABEL[filter as Exclude<FilterTab, "reports">]}</p>
          </div>
        )}

        {filter === "pending" && !currentTab?.isLoading && !currentTab?.isError && (
        ...
        {filter === "registered" && !currentTab?.isLoading && !currentTab?.isError && (
          <div className="space-y-3">
            {cafes.map((cafe) => (
              <RegisteredCafeCard
                key={cafe.id}
        ...
        {filter === "images" && !currentTab?.isLoading && !currentTab?.isError && (
          <div className="space-y-3">
        ...
        {filter === "edits" && !currentTab?.isLoading && !currentTab?.isError && (
          <div className="space-y-3">
            {editSubmissions.map((submission) => (
              <EditSubmissionCard
                ...
                isDeleting={
                  rejectEdit.isPending && rejectEdit.variables === submission.id
                }
              />
            ))}
          </div>
